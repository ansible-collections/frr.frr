...
class TestFrrBgpModule(TestFrrModule):
    ...
    def test_frr_bgp_address_family_networks(self):
        ...
        cmd = [
            "router bgp 64496",
            "address-family ipv4 multicast",
            "network 1.0.0.0/8 route-map RMAP_1",
            "network 192.168.1.0/24 route-map RMAP_2",
            "exit-address-family",
            "exit",
        ]
        self.assertEqual(sorted(commands), sorted(cmd))
    ...
    def test_frr_bgp_operation_override(self):
        ...
        cmd = [
            ...
            "neighbor 192.51.100.1 advertisement-interval 120",
            "neighbor 192.51.100.3 remote-as 64496",
            "neighbor 192.51.100.3 timers 300 360",
            "address-family ipv4 unicast",
            "neighbor 192.51.100.1 maximum-prefix 35",
            "neighbor 192.51.100.3 route-reflector-client",
            "exit-address-family",
